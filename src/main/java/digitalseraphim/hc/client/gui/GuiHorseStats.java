...
public class GuiHorseStats extends GuiScreen implements IProgressMeter{
    private static final int leftEdge = AchievementList.minDisplayColumn * 24 - 112;
    private static final int topEdge = AchievementList.minDisplayRow * 24 - 112;
    private static final int rightEdge = AchievementList.maxDisplayColumn * 24 - 77;
    private static final int bottomEdge = AchievementList.maxDisplayRow * 24 - 77;
    ...
    public GuiHorseStats(GuiScreen p_i45026_1_, StatFileWriter p_i45026_2_)
    {
    ...
    public void drawScreen(int mouseX, int mouseY, float renderPartialTicks)
    {
        ...
        else
        {
            ...
            if (Mouse.isButtonDown(0))
            {
                ...
                if ((this.mouse0WasDown == 0 || this.mouse0WasDown == 1) && mouseX >= i1 && mouseX < i1 + 224 && mouseY >= j1 && mouseY < j1 + 155)
                {
                    if (this.mouse0WasDown == 0)
                    {
                        this.mouse0WasDown = 1;
                    }
                    else
                    {
                        this.field_146567_u -= (double)((float)(mouseX - this.lastMouseX) * this.zoom);
                        this.field_146566_v -= (double)((float)(mouseY - this.lastMouseY) * this.zoom);
            ...
            {
                float f6 = lastZoom - this.zoom;
                float f5 = lastZoom * (float)this.field_146555_f;
                float f1 = lastZoom * (float)this.field_146557_g;
                float f2 = this.zoom * (float)this.field_146555_f;
                float f3 = this.zoom * (float)this.field_146557_g;
            ...
            if (this.field_146565_w < (double)leftEdge)
            {
                this.field_146565_w = (double)leftEdge;
            }

            if (this.field_146573_x < (double)topEdge)
            {
                this.field_146573_x = (double)topEdge;
            }

            if (this.field_146565_w >= (double)rightEdge)
            {
                this.field_146565_w = (double)(rightEdge - 1);
            }

            if (this.field_146573_x >= (double)bottomEdge)
            {
                this.field_146573_x = (double)(bottomEdge - 1);
            }

            this.drawDefaultBackground();
            this.func_146552_b(mouseX, mouseY, renderPartialTicks);
    ...
    protected void func_146552_b(int p_146552_1_, int p_146552_2_, float p_146552_3_)
    {
        ...
        GL11.glScalef(1.0F / this.zoom, 1.0F / this.zoom, 0.0F);
        ...
        float f4 = (float)(p_146552_1_ - k1) * this.zoom;
        float f5 = (float)(p_146552_2_ - l1) * this.zoom;
        ...
        for (l4 = 0; l4 < achievementList.size(); ++l4)
        {
            ...
            if (i5 >= -24 && j5 >= -24 && (float)i5 <= 224.0F * this.zoom && (float)j5 <= 155.0F * this.zoom)
            {
